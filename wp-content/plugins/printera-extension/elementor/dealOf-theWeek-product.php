<?php
/**
 * Widget API: Elementer Deal Of The Week Widget
 *
 * @package printera
 * @subpackage Widgets
 * @since 1.0.0
...
/**
 * Elementor Deal Of The Week widget.
 * Elementor widget that displays a Deal Of The Week with the ability to control every
 * aspect of the Deal Of The Week design.
 * @since 1.0.0
 */
class Widget_printera_DealOf_TheWeek extends Widget_Base {
	/**
	 * Get widget name.
	 * Retrieve Deal Of The Week widget name.
	 * @since 1.0.0
	 * @access public
	 * @return string Widget name.
	 */
	public function get_name() {
		return esc_html__( 'Deal Of The Week', 'printera' );
	}

	/**
	 * Get widget title.
	 * Retrieve Deal Of The Week widget title.
	 * @since 1.0.0
	 * @access public
	...
	/**
	 * Get widget icon.
	 * Retrieve Deal Of The Week widget icon.
	 * @since 1.0.0
	 * @access public
	...
	/**
	 * Register Deal Of The Week widget controls.
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			'Deal Of The Week',
			array(
				'label' => esc_html__( 'Deal Of The Week', 'printera' ),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'label_block' => true,
				'default' => esc_html__( 'Deal Of The Weeks', 'printera' ),
			)
		);
		...
		$this->end_controls_section();
		$this->start_controls_section(
			'Deal Of The Week_style',
			array(
				'label' => esc_html__( 'Deal Of The Week Style', 'printera' ),
				'tab'   => Controls_Manager::TAB_STYLE,
			)
		);
	...
	/**
	 * Render Deal Of The Week widget output on the frontend.
	...
	protected function render() {
		...
		/* $this->add_render_attribute(
			'category-carousel',
			[
				'class' => ['swiper productdeal-weekg' ]
			]
		); */
		...
		$this->add_render_attribute(
			'attribute',
			[
				'class' => ['product-deal-week' , $settings['row-style'] , $stock ]
			]
		);
		...
		?>
				<div <?php echo $this->get_render_attribute_string( 'attribute' ); ?> id="product-deal-week">
					...
					<div class="title-wrap">
						<?php if( ! empty( $settings['trending-title'] ) ){ ?>
							...
							<?php if( ! empty( $date_time ) ){ ?><div class="timer-date" id="timer-date" data="<?php echo $date_time; ?>">
								<div>
									<p id="days" class="days"><div class="text">Days</div></p>
								</div>
								<div>
									<p id="hours" class="hours"><div class="text">Hours</div></p>
								</div>
								<div>
									<p id="minutes" class="minutes"><div class="text">Mins</div></p>
								</div>
								<div>
									<p id="second" class="second"><div class="text">Secs</div></p>
								</div>
							</div><?php } ?>
						<?php } ?>
					</div>
					...
					<div class="deal-wrap">
						<div class="swiper products-deal deal-week deal-swiper <?php echo $settings['hover-style']; ?>" data-id = "deal-week" id = "deal-week" >
		...
		if ( Plugin::$instance->editor->is_edit_mode() ) :
			?>
			<script>
				if( jQuery('div').hasClass('products-deal') ){

						$this = jQuery('.swiper.products-deal');
						...
						mySwiper = new Swiper('.swiper.products-deal', {
							direction: direction,
							spaceBetween: margin,
							loop: loop,
							autoplay: autoplay,
...
Plugin::instance()->widgets_manager->register_widget_type( new \Elementor\Widget_printera_DealOf_TheWeek() );
